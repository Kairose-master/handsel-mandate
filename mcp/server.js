...
export function buildServer(env = process.env, deps = {}) {
  ...
  server.registerTool('discover', { title: '상품 찾기', description: `Search the x402 Bazaar (thousands of listed pay-per-call APIs) plus registered seller catalogs for tools matching a task on ${net.name}. Returns url, method, price (USDC per call), exampleRequest (POST body), exampleQuery (GET params), exampleResponse. Prefer the cheapest item that fits; never buy anything not returned here.`, inputSchema: { query: z.string().describe('What the human needs, e.g. "markdown 표를 JSON으로"'), limit: z.number().int().min(1).max(25).optional() } }, async ({ query, limit }) => text(await catalog.discover(query, { limit })));
  server.registerTool('buy', { title: '구매', description: `Pay one x402 quote inside the mandate and return the tool output. Refuses anything over the per-call limit or remaining budget, other networks, or sellers outside the allowlist. Spends ${net.testnet ? 'testnet' : 'REAL'} USDC.`, inputSchema: { url: z.string().url().describe('Product URL from discover'), method: z.enum(['GET', 'POST']).optional(), input: z.record(z.string(), z.unknown()).optional().describe('JSON body for POST tools (shape from exampleRequest)'), query: z.record(z.string(), z.string()).optional().describe('Query parameters for GET tools (keys from exampleQuery)'), request_id: z.string().regex(/^[\w-]{1,100}$/).optional().describe('Idempotency key; reuse to retry without paying twice') } }, async ({ url, method, input, query, request_id }) => {
    try {
      const r = await wallet.buy({ url, method, body: input, query, requestId: request_id });
