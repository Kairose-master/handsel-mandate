...
test('catalog normalizes seller product.json and filters to the configured network', async () => {
  ...
  const catalog = createCatalog({ network: 'eip155:8453', catalogUrls: ['https://s.example/product.json'], fetcher: async () => new Response(JSON.stringify(product)) });
  assert.deepEqual((await catalog.discover('표')).items, [], 'testnet product hidden on mainnet');
  const cat2 = createCatalog({ network: 'eip155:84532', catalogUrls: ['https://s.example/product.json', 'https://down.example/p.json'], fetcher: async url => url.includes('down') ? new Response('', { status: 500 }) : new Response(JSON.stringify(product)) });
...
test('MCP over stdio: delegate → discover → buy within budget → blocked over budget → receipts', async () => {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), '402lab-'));
  const transport = new StdioClientTransport({ command: process.execPath, args: ['mcp/server.js'], env: { ...process.env, BUYER_PRIVATE_KEY: generatePrivateKey(), NETWORK: 'eip155:84532', CATALOG_URLS: `${base}/product.json`, STATE_PATH: path.join(dir, 'state.json'), MANDATE_MAX_USDC: '1' }, stderr: 'pipe' });
...
test('MCP over HTTP: bearer token and extension-only origin, driven by the browser client', async () => {
  const { demo, base } = await seller();
  const dir = await mkdtemp(path.join(tmpdir(), '402lab-'));
  const built = buildServer({ BUYER_PRIVATE_KEY: generatePrivateKey(), NETWORK: 'eip155:84532', CATALOG_URLS: `${base}/product.json`, STATE_PATH: path.join(dir, 'state.json') });
